...
import PlantillasAPEX3.*;
import PlantillasSPPR300.*;
...
public class BluetoothPlugin extends CordovaPlugin {
	...
	private void action_write(int socketId, JSONObject impresion,CallbackContext CallbackContext) {
		...
		if(impresion.optString("impresoraEnUso").equals("APEX3")){
			socket = m_bluetoothSockets.get(socketId);
		}
		...
		try {
			...
			if (impresion.optString("tipoBoucher").equals("FACTURACION")) {
				...
				if(impresion.optString("impresoraEnUso").equals("APEX3")){
					...
					if (impresion.optString("tituloFactura").equals("FACTURA")) {
						...
						plantilla.setnumeroCopia(Integer.parseInt(impresion.optString("noCopia")));
				...
				else if(impresion.optString("impresoraEnUso").equals("SPP-R300")  || impresion.optString("impresoraEnUso").equals("SPP-R310")  ){
					...
					plantilla.setRucCliente(impresion.optString("rucCliente"));
					plantilla.setDatosLineas(impresion.optJSONArray("detalleFactura"));
					plantilla.setPorcentajeCentralizacion(impresion.optString("porcentajeCentralizacion"));
					...
					if (impresion.optString("tituloFactura").equals("FACTURA")) {
						...
						CallbackContext.success(plantilla.getFactura());
						return;

					} else if (impresion.optString("tituloFactura").equals("CAMBIO")) {
						plantilla.setCopias(Integer.parseInt(impresion.optString("copias")));

						CallbackContext.success(plantilla.getFactCambio());
						return;

					} else if (impresion.optString("tituloFactura").equals("DEVOLUCION")) {
						plantilla.setCopias(Integer.parseInt(impresion.optString("copias")));

						CallbackContext.success(plantilla.getFactDevolucion());
						return;
					}
				}
				else{
				...
				if(impresion.optString("impresoraEnUso").equals("APEX3")){
					...
					//	setBuffer(pmMinutaDeposito.getMinutaDeposito().getBytes());
					CallbackContext.success(pmMinutaDeposito.getMinutaDeposito());
					return;
				}
				else{
					//	pluginResult = new PluginResult(PluginResult.Status.ERROR,"Dispositivo invalido: "+impresion.optString("impresoraEnUso"));
			...
			}else if(impresion.optString("tipoBoucher").equals("RESUMEN_PAGO"))
			{
				if(impresion.optString("impresoraEnUso").equals("APEX3")){
					...
					//	setBuffer(pmResumenPago.getResumenPago().getBytes());
					CallbackContext.success(pmResumenPago.getResumenPago());
					return;
				}
				else{
					//pluginResult = new PluginResult(PluginResult.Status.ERROR,"Dispositivo invalido: "+impresion.optString("impresoraEnUso"));
				...
				if(impresion.optString("impresoraEnUso").equals("APEX3")){
					...
					//	setBuffer(pmMinutaDeposito.getResumenDia().getBytes());
					CallbackContext.success(pmMinutaDeposito.getResumenDia());
					return;
				}
				else{
					//pluginResult = new PluginResult(PluginResult.Status.ERROR,"Dispositivo invalido: "+impresion.optString("impresoraEnUso"));
				...
				if(impresion.optString("impresoraEnUso").equals("APEX3")){
					...
					//setBuffer(pmInventarioABordo.getInventarioABordo().getBytes());
					CallbackContext.success(pmInventarioABordo.getInventarioABordo());
					return;
				}
				else{
					//pluginResult = new PluginResult(PluginResult.Status.ERROR,"Dispositivo invalido: "+impresion.optString("impresoraEnUso"));
				...
				if(impresion.optString("impresoraEnUso").equals("APEX3")){
					...
					prReciboPago.setnumeroCopia(Integer.parseInt(impresion.optString("noCopia")));
					...
					//	CallbackContext.success("OK");
					//	return;
					CallbackContext.success(prReciboPago.getReciboPago());
					return;
				}
				else{
					//pluginResult = new PluginResult(PluginResult.Status.ERROR,"Dispositivo invalido: "+impresion.optString("impresoraEnUso"));
				...
				if(impresion.optString("impresoraEnUso").equals("APEX3")){
					...
					prReciboPagoEnvase.setnumeroCopia(Integer.parseInt(impresion.optString("noCopia")));
					...
					//	CallbackContext.success("OK");
					//	return;
					CallbackContext.success(prReciboPagoEnvase.getReciboPagoEnvases());
					return;
				}
				else{
					//pluginResult = new PluginResult(PluginResult.Status.ERROR,"Dispositivo invalido: "+impresion.optString("impresoraEnUso"));
			...
			else if (impresion.optString("tipoBoucher").equals("DOCENTREGA")) {

				if(impresion.optString("impresoraEnUso").equals("SPP-R300")  || impresion.optString("impresoraEnUso").equals("SPP-R310")  ){

					PlantillaDOCENTREGA plantilla = new PlantillaDOCENTREGA();

					plantilla.setCodigoCliente(impresion.optString("codigoCliente"));
					plantilla.setCliente(impresion.optString("cliente"));
					plantilla.setRucCliente(impresion.optString("rucCliente"));
					plantilla.setFecha(impresion.optString("fecha"));
					plantilla.setPedido(impresion.optString("pedido"));
					...
					plantilla.setIVA(impresion.optString("IVA"));
					plantilla.setTotal(impresion.optString("total"));
					plantilla.setDetalle(impresion.optJSONArray("detalle"));

					CallbackContext.success(plantilla.getDocEntrega());
					return;
				}
				else{
					CallbackContext.error("Dispositivo invalido: "+impresion.optString("impresoraEnUso"));
